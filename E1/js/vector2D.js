...
class Vector2D
{
    ...
    /**
     * Returns a vector with random orientation and magnitude of 1.
     */
    static Random()
    {
        //Generate a random angle.
        let rand = Math.random() * TWO_PI;
        //Return the normalized polar vector of that angle.
        return new Vector2D(sin(rand), cos(rand));
    }
     ...
     * If two scaling factors are given, then the first will be used on x and the second on y.
     * If a single scaling factor is given, it will be used on both x and y.
     * @param {number} scalingFactor1 The amount to scale x or the whole vector.
     * @param {number} scalingFactor2 The amount to scale y. (optional)
     */
    Scale(scalingFactor1, scalingFactor2 = scalingFactor1)
    {
        return new Vector2D(this.x * scalingFactor1, this.y * scalingFactor2);
    }
}
